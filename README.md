# English Learning Site - Static Version

英語学習サイトの静的バージョンです。GitHub Pagesで動作するように最適化されています。**大学受験（センター試験、国公立大学、私立大学）向けの高度な文法問題**を提供しています。
...
このサイトは静的サイトとして動作するため、特別なセットアップは不要です。
...
3. ブランチを`gh-pages`に設定

## 静的サイト機能

このサイトは静的サイトとして動作し、以下の機能を提供します：

### 問題提供
- 品詞、文型、時制、助動詞、仮定法、比較などの文法問題
- 英作文問題
- 和文英訳問題
- 詳細な解説付き
...
- 静的サイトのため、サーバーサイドのセキュリティリスクなし
- すべての処理がクライアントサイドで実行
- プライバシー保護（データはローカルに保存）
...
```
├── index.html             # メインページ
├── static-data.js         # 静的データファイル
├── script.js              # クライアントサイドJavaScript
├── style.css              # スタイルシート
├── grammar.json           # 文法データ
├── components/            # 再利用可能なコンポーネント
├── parts-of-speech/       # 品詞の学習ページ
├── sentence-patterns/     # 文型の学習ページ
├── phrases/               # 句の学習ページ
├── clauses/               # 節の学習ページ
├── tenses/                # 時制の学習ページ
├── modals/                # 助動詞の学習ページ
├── subjunctive/           # 仮定法の学習ページ
├── comparison/            # 比較の学習ページ
└── README.md             # このファイル
```
